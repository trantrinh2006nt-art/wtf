...
class VERPipeline:
    ...
    def _init_retriever(self):
        ...
        try:
            ...
            # Fallback: dùng các engine riêng lẻ cũ
            log_warning(
                "MultiIndexRetriever chưa tồn tại. "
                "Fallback sang các engine riêng lẻ."
            )
            return self._init_fallback_retriever()
        except Exception as e:
            log_error(f"MultiIndexRetriever: Lỗi — {e}")
            return self._init_fallback_retriever()

    def _init_fallback_retriever(self):
        """Fallback: khởi tạo text + visual engines riêng lẻ."""
        retriever = _FallbackRetriever()

        # Text engine
        try:
            from text_engine import TextRetrievalEngine
            retriever.text_engine = TextRetrievalEngine()
            retriever.text_engine.load_index()
            log_success("TextRetrievalEngine (fallback): ✓")
        except Exception as e:
            log_warning(f"TextRetrievalEngine fallback lỗi: {e}")

        # Visual engine
        try:
            from visual_engine import VisualRetrievalEngine
            retriever.visual_engine = VisualRetrievalEngine()
            retriever.visual_engine.load_index()
            log_success("VisualRetrievalEngine (fallback): ✓")
        except Exception as e:
            log_warning(f"VisualRetrievalEngine fallback lỗi: {e}")

        # Fusion engine
        try:
            from fusion import CrossModalFusion
            retriever.fusion = CrossModalFusion()
            log_success("CrossModalFusion (fallback): ✓")
        except Exception as e:
            log_warning(f"CrossModalFusion fallback lỗi: {e}")
    ...
    def _retrieve(self, analyzed_query: AnalyzedQuery) -> list[SearchResult]:
        ...
        try:
            ...
            elif hasattr(self.retriever, 'text_engine'):
                # Fallback retriever
                results = self.retriever.retrieve_fallback(
                    analyzed_query.original_query, top_k=TOP_K_RETRIEVAL
                )
            else:
    ...
    def _preprocess_single_video(self, video_path: str) -> list[SegmentSchema]:
        ...
        segments = self._temporal_align(
            video_name, duration, asr_segments, audio_events, ocr_data, video_clips, frame_data
        )
    ...
    def _build_all_indexes(self, segments: list[SegmentSchema]):
        ...
        try:
            # Sửa tên class thành UnifiedIndexer để khớp với file indexer.py của em
            from offline.indexer import UnifiedIndexer
            builder = UnifiedIndexer()
            
            # Gọi đúng tên hàm build_all_indexes
            builder.build_all_indexes(segments)
            log_success("Đã build tất cả indexes")
        except ImportError:
            log_warning("UnifiedIndexer chưa tồn tại — thử build riêng lẻ")
            self._build_indexes_fallback(segments)
        except Exception as e:
            log_error(f"Build indexes lỗi: {e}")

    def _build_indexes_fallback(self, segments: list[SegmentSchema]):
        """Fallback: build từng index riêng lẻ."""
        # Text index
        try:
            from text_engine import TextRetrievalEngine
            engine = TextRetrievalEngine()
            seg_dicts = [
                {"start": s.start_time, "end": s.end_time, "text": s.get_full_text()}
                for s in segments if s.get_full_text().strip()
            ]
            if seg_dicts:
                engine.build_index(seg_dicts, segments[0].video_file if segments else "unknown")
                log_success(f"Text index: {len(seg_dicts)} documents")
        except Exception as e:
            log_error(f"Build text index lỗi: {e}")

        # Visual index
        try:
            from visual_engine import VisualRetrievalEngine
            engine = VisualRetrievalEngine()
            video_files = list(set(s.video_file for s in segments))
            for vf in video_files:
                vpath = str(RAW_VIDEO_DIR / vf)
                if os.path.exists(vpath):
                    engine.build_index(vpath)
            log_success(f"Visual index: {len(video_files)} videos")
        except Exception as e:
            log_error(f"Build visual index lỗi: {e}")
...
# ============================================================
# FALLBACK RETRIEVER — Dùng khi MultiIndexRetriever chưa tồn tại
# ============================================================

class _FallbackRetriever:
    """
    Retriever đơn giản gộp text + visual + fusion.
    Dùng khi online/retriever.py chưa được viết.
    """
    ...
    def retrieve_fallback(self, query: str, top_k: int = TOP_K_RETRIEVAL) -> list[SearchResult]:
        """Truy xuất cơ bản: text + visual → fusion."""
        text_results = []
        visual_results = []
        ...
        if self.text_engine is not None:
            try:
                text_results = self.text_engine.search(query, top_k=top_k)
                log_info(f"Text: {len(text_results)} kết quả")
            except Exception as e:
                log_error(f"Text search lỗi: {e}")

        # Visual retrieval
        if self.visual_engine is not None:
            try:
                visual_results = self.visual_engine.search(query, top_k=top_k)
                log_info(f"Visual: {len(visual_results)} kết quả")
            except Exception as e:
                log_error(f"Visual search lỗi: {e}")

        # Fusion
        if self.fusion is not None and text_results and visual_results:
            try:
                return self.fusion.fuse(text_results, visual_results)
            except Exception as e:
                log_error(f"Fusion lỗi: {e}")

        # Fallback: gộp và sắp xếp
